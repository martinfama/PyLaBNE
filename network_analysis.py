...
#x-axis: k
#y-axis: p(k)
def graph_degree_plot(graph=igraph.Graph, logScale=False, legend=True, xmin=None):
    ...
    degrees = np.sort(graph.degree())[::-1]
    counts, bin_edges = np.histogram(degrees, bins=np.logspace(0, np.log10(max(degrees)), 50), density=True) 
    bin_centres = (bin_edges[:-1] + bin_edges[1:])/2
    sc = ax.scatter(bin_centres, counts, s=6)
...
#plot inferred angle vs real angle
#function takes a graph which should be generated by the PS model,
#and the embedding by LaBNE, also given as a graph with updated coordinates
#returns figure and axis
def inferred_angles(PS_graph=None, LaBNE_graph=None, colors=False):
    ...
    if colors == True:
        ...
        ax.scatter(PS_graph.vs['theta'], LaBNE_theta, s=4, c='firebrick')
    #ax.scatter(PS_graph.vs['theta'], LaBNE_theta, s=6)

    return fig, ax

#apply precision recall by using a network with two snapshots. The second snapshot should be trimmed, i.e. any new
#nodes shouldn't be included, only the new edges between existing nodes. metric defines what score index to use,
#which is basically one of 5 neighbourhood-based metrics or LaBNE.
def precision_recall_snapshots(snapshot_t0 = igraph.Graph(), snapshot_t1 = igraph.Graph(), metric=None, step=1):
...
#same as precision_recall_snapshots, but for large graphs where memory is an issue. Here, the link scores are saved to a file,
#which is then sorted using Unix sort command. 
def precision_recall_snapshot_files(snapshot_t0 = ng.Network, snapshot_t1 = ng.Network, header='', metric=None, debug_print_flag=False):
    ...
    PR_LP_sorted = open(f'./obj/{header}_PR_{metric}_sorted_final.csv')
    tp = 0
    fp = 0
    ...
    C_matrix = []
    non_train_edges = fn
    total_edges = int(N*(N-1)/2-snapshot_t0.graph.ecount())
    for i in range(0, total_edges):
        ...
        if l[2] != '0.0':
            ...
            if i % step == 0:
                ...
                if recall[-1] == 1:
                    print(i, i/total_edges, total_edges)
    return precision, recall, C_matrix

#precision recall applied to a train set, which is created by randomly removing edges. The method is a bit slow, since just randomly
#removing edges can end up disconnecting the graph (i.e. making subgraphs), so we need to make sure that the graph is connected
def precision_recall_train_set(graph=igraph.Graph(), metric=None, test_size=0.33, random_state=47, step=1, debug_print_flag=False):
...
# possible link prediction with regression model
