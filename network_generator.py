...
def graph_degree_bins(graph=None):
    #graph_bins = np.transpose(list(graph.degree_distribution().bins()))
    graph_bins = np.transpose([x for x in list(graph.degree_distribution().bins())[1:] if x[2] > 1])
    graph_bins = [graph_bins[0], graph_bins[2]/sum(graph_bins[2])]
    return graph_bins
...
class Network(object):
    def __init__(self, **kwargs):
        ...
        #receive an edgelist as a filename, and create a Graph() with that
            ...
            #self.graph = igraph.Graph.Read_Ncol(kwargs.get('edgelist'), directed=False)
    ...
    def infer_gamma(self, inplace=False):
        #gamma = igraph.power_law_fit(data=self.graph.degree(), xmin=2, method='hill').alpha
        avg_k = self.infer_avg_k()
        gamma = igraph.power_law_fit(data=np.sort(self.graph.degree())[::-1], xmin=8*np.log10(avg_k)).alpha
...
def generatePSNetwork(N=0, avg_k=0, gamma=0, Temp=0, seed=0):
    ...
    for t in range(1, N):
        ...
        if t <= m:
            ...
            #we keep a list of nodes to which the new node is not connected
            s_unconnected = [s for s in range(t)]
            ...
            # if Temp > 0, the new node connects to m nodes randomly, with a probability
            # distribution that depends on the distance between the new node and the candidate nodes
            elif Temp > 0:
...
def generateLaBNE(network=None, graph=None, v_k=2, scatterPlot=False, plotEdges=False, xy=False):

    #check which object is being passed, network or graph (or none)
    #network overrides graph
    if network == None:
        if graph == None:
            raise Exception("No network or graph object passed.")
        else:
    ...
    if N < 10000:
        eigenvalues, eigenvectors = scipy.sparse.linalg.eigs(A=L, M=D, k=v_k+1, which='LM', sigma=0, return_eigenvectors=True)
        #eigenvalues, eigenvectors = scipy.sparse.linalg.eigs(A=L, M=np.diag(graph.degree()), k=eigenvector_k, which='LM', sigma=0, return_eigenvectors=True, maxiter=100000)
        """
        except(RuntimeError):
            print('Levenberg')
            ...
            eigenvalues, eigenvectors = scipy.sparse.linalg.eigs(A=L+Levenberg_c, M=M, k=eigenvector_k, which='LM', sigma=0, return_eigenvectors=True, maxiter=100000)
        """
        #eigenvalues, eigenvectors = scipy.sparse.linalg.eigs(A=L, k=eigenvector_k, which='SM', return_eigenvectors=True)
    else:        
        eigenvalues, eigenvectors = scipy.sparse.linalg.eigs(A=L, M=D, k=v_k+1, which='LM', sigma=0, return_eigenvectors=True, tol=1E-7, maxiter=10000)
        #except(RuntimeError):
        #    Levenberg_c = np.zeros(np.shape(L))
        #    np.fill_diagonal(Levenberg_c, 0.01)
        #    Levenberg_c = scipy.sparse.coo_matrix(Levenberg_c)
        #    eigenvalues, eigenvectors = scipy.sparse.linalg.eigs(A=L+Levenberg_c, M=M, k=eigenvector_k, which='LM', sigma=0, return_eigenvectors=True, tol=1E-7, maxiter=10000)

        #eigenvalues, eigenvectors = scipy.sparse.linalg.eigs(A=L, k=eigenvector_k, which='SM', return_eigenvectors=True, tol=1E-7)
    eigenvectors = np.transpose(eigenvectors)
    x, y = eigenvectors[2].real, eigenvectors[1].real
